...
describe LogtoMobile::UserProvisioner do
  ...
  before { SiteSetting.logto_mobile_session_auto_approve_users = true }
  ...
  describe "#provision" do
    it "creates a new activated Discourse user with associated OIDC account" do
      ...
      associated_account = UserAssociatedAccount.find_by(user_id: user.id, provider_name: "oidc")
      ...
      expect(Jobs).to have_received(:enqueue).with(
        ...
        satisfy do |payload|
          payload[:url] == "https://cdn.example.com/avatar.png" && payload[:user_id] == user.id &&
    ...
    it "updates an existing user matched by email" do
      existing_user =
        Fabricate(:user, email: "existing@example.com", name: "Old Name", username: "existing")
      allow(Jobs).to receive(:enqueue)
      ...
      expect(Jobs).to have_received(:enqueue).with(
        :change_display_name,
        satisfy do |payload|
          payload[:new_name] == "Updated Name" && payload[:old_name] == "Old Name"
        end,
      )
      expect(Jobs).to have_received(:enqueue).with(
        :download_avatar_from_url,
    ...
    it "creates UserAssociatedAccount for existing user without one" do
      ...
      association = UserAssociatedAccount.find_by(user_id: existing_user.id, provider_name: "oidc")
    ...
    context "when duplicate association handling" do
      it "destroys old association when user switches Logto accounts" do
        ...
        new_association = UserAssociatedAccount.find_by(user_id: user.id, provider_name: "oidc")
        expect(new_association).to be_present
        expect(new_association.provider_uid).to eq("new-logto-sub-123")
      end
    end

    context "when UserAssociatedAccount field population" do
      it "populates all required fields correctly" do
        allow(Jobs).to receive(:enqueue)

        user = provisioner.provision

        association = UserAssociatedAccount.find_by(user_id: user.id, provider_name: "oidc")
    ...
    context "when staged user conversion" do
    ...
    context "when profile sync" do
      ...
      it "does not override existing bio and location" do
        existing_user = Fabricate(:user, email: "existing@example.com")
        existing_user.user_profile.update!(bio_raw: "Existing bio", location: "New York")
        allow(Jobs).to receive(:enqueue)
    ...
    context "when avatar override settings" do
      it "respects auth_overrides_avatar setting when false" do
        ...
        allow_any_instance_of(User).to receive_message_chain(
          :user_avatar,
          :custom_upload_id,
        ).and_return(123)
        allow(Jobs).to receive(:enqueue)
